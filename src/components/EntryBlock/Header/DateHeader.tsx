  ...
  const [from, to] = useMemo(() => {
    ...
    if (!data.endDate && !data.date) {
      ...
      if (start.getFullYear() === end.getFullYear()) {
        if (start.getMonth() === end.getMonth()) from = `${start.toLocaleString("default", { month: "short" })} ${start.getFullYear()}`;
        else {
          from = start.toLocaleString("default", { month: "short" });
          to = `${end.toLocaleString("default", { month: "short" })} ${end.getFullYear()}`;
        }
      } else {
